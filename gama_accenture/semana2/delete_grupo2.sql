...
SELECT COUNT(DISTINCT NO_ESTADO) FROM TB05_ENDERECO;

SELECT * FROM TB01_AGENCIA WHERE UPPER(NO_AGENCIA) LIKE '%C%';

SELECT NO_LOGRADOURO || ', ' || NO_NUMERO || ' - ' || NO_CIDADE || '/' || NO_ESTADO as "Endereço" FROM TB05_ENDERECO;

SELECT * FROM TB03_CLIENTE WHERE upper(NO_CLIENTE) LIKE '%VICTOR%';